...
opclRR = 0     #/* reg operands r,s,t */
opclRM = 1     #/* reg r, mem d+s */
opclRA = 2      #/* reg r, int d+s */

opHALT = 0    #/* RR     halt, operands are ignored */
opIN = 1      #/* RR     read into reg(r); s and t are ignored */
opOUT = 2     #/* RR     write from reg(r), s and t are ignored */
opADD = 3    #/* RR     reg(r) = reg(s)+reg(t) */
opSUB = 4,    #/* RR     reg(r) = reg(s)-reg(t) */
opMUL = 5    #/* RR     reg(r) = reg(s)*reg(t) */
opDIV = 6    #/* RR     reg(r) = reg(s)/reg(t) */
opRRLim = 7   #/* limit of RR opcodes */

opLD = 8      #/* RM     reg(r) = mem(d+reg(s)) */
opST = 9      #/* RM     mem(d+reg(s)) = reg(r) */
opRMLim = 10   #/* Limit of RM opcodes */

opLDA = 11     #/* RA     reg(r) = d+reg(s) */
opLDC = 12     #/* RA     reg(r) = d ; reg(s) is ignored */
opJLT = 13     #/* RA     if reg(r)<0 then reg(7) = d+reg(s) */
opJLE = 14     #/* RA     if reg(r)<=0 then reg(7) = d+reg(s) */
opJGT = 15     #/* RA     if reg(r)>0 then reg(7) = d+reg(s) */
opJGE = 16     #/* RA     if reg(r)>=0 then reg(7) = d+reg(s) */
opJEQ = 17     #/* RA     if reg(r)==0 then reg(7) = d+reg(s) */
opJNE = 18     #/* RA     if reg(r)!=0 then reg(7) = d+reg(s) */
opRALim = 19    #/* Limit of RA opcodes */
